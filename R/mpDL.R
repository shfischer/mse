...
mpDL <- function(om, oem=FLoem(), iem=NULL, ctrl.mp, genArgs, scenario="test", tracking="missing", verbose=TRUE){
  ...
  ### set seed per part?
	seed_part <- ifelse(!is.null(genArgs$seed_part), genArgs$seed_part, FALSE)
  ...
  if (exists(fleetBehaviour(om))) fb <- fleetBehaviour(om) else fb <- NULL 
	...
	if(nblocks > 1){
		...
		# LOOP and combine
		lst0 <- foreach(i=its, i_part = seq_along(its),
                    .combine=function(...) {
			list(stk.om=do.call('combine_attr', lapply(list(...), '[[', 'stk.om')),
        ...
        if (isTRUE(seed_part)) set.seed(i_part)
			# SUBSET object(s)
			call0 <- list(
				###stk.om = stk.om[,,,,,i],
				stk.om = iter_attr(stk.om, i),
...
#' @export
goFishDL <- function(stk.om, sr.om, sr.om.res, sr.om.res.mult, fb, projection, oem, iem, tracking, ctrl.mp, genArgs, verbose){

	### it <- genArgs$it
 	it <- dims(stk.om)$iter ### use stk.om because it might have been subset
