...
#' @export
mpDL <- function(om, oem=NULL, iem=NULL, ctrl, args, scenario="test", tracking="missing", verbose=TRUE, cut_hist = TRUE, ...){
  ...
  ### set seed per part?
	seed_part <- ifelse(!is.null(args$seed_part), args$seed_part, FALSE)
	...
	### start tracking object earlier so that historical values are available
  tracking <- FLQuant(NA, dimnames = list(metric = metric,
                                          year = c(dims(stk.om)$minyear:fy), 
                                          iter = 1:it))
    ...
    cat("Going parallel!\n")
		...
		lst0 <- foreach(j = it_blocks, j_part = seq_along(it_blocks),
                    .combine=function(...) {
			list(
        stk.om=do.call('combine_attr', lapply(list(...), '[[', 'stk.om')),
      ...
      .errorhandling = "stop", 
      .inorder=TRUE) %dopar% {
				# set iter for current block
				args$it <- length(j)
        ### set seed for current part, if requested
        if (isTRUE(seed_part)) set.seed(j_part)
			# SUBSET object(s)
			call0 <- list(
				###stk.om = stk.om[,,,,,j],
				...
				iem=iters(iem, j), # needs it selection
				ctrl= iters(ctrl, j),
				args=args,
				verbose=verbose)
			out <- do.call(goFishDL, call0)
			# RETURN
...
#' @export
goFishDL <- function(stk.om, sr.om, sr.om.res, sr.om.res.mult, fb, projection, oem, iem, tracking, ctrl, args, verbose){
